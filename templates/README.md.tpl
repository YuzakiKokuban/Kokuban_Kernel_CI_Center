...
* **扩展特性支持**：可按 workflow 构建配置集成 `Hybrid Mount`、`SuSFS` 与 `BBG`；自动构建默认不集成适用于所有内核的 `Hybrid Mount`，`SuSFS` 仅在 `ReSukiSU` 构建中启用。
...
* **Optional Feature Integration**: Automated builds leave `Hybrid Mount` disabled by default and expose a workflow option to enable it. Matching builds may also include `SuSFS` and `BBG`, with `SuSFS` enabled only on `ReSukiSU` releases.

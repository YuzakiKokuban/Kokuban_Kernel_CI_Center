...
* **扩展特性支持**：默认集成适用于所有内核的 `Hybrid Mount`，并可按构建配置集成 `SuSFS` 与 `BBG`；其中 `SuSFS` 仅在 `ReSukiSU` 构建中启用。
...
* **Optional Feature Integration**: `Hybrid Mount` is enabled for all kernels by default. Matching builds may also include `SuSFS` and `BBG`, with `SuSFS` enabled only on `ReSukiSU` releases.

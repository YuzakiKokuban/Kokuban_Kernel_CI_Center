...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Build {
            ...
            branch,
            do_release,
        } => handle_build(project, branch, do_release),
    }
}
...
fn handle_build(project_key: String, branch: String, do_release: bool) -> Result<()> {
    let projects = load_projects()?;
    let proj_val = projects
        .get(&project_key)
        .ok_or_else(|| anyhow!("Project not found"))?;
    ...
    if let Some(urls) = &proj.toolchain_urls {
        ...
        for entry in fs::read_dir(&tc_download_dir)? {
            let entry = entry?;
            let path = entry.path();
            if let Some(name) = path.file_name() {
                let name_str = name.to_string_lossy();
                if name_str.contains(".tar.gz") {
                    println!("Extracting {}", name_str);
                    run_cmd(
                        &[
                            "tar",
                            "-zxf",

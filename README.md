...
* **动态特性注入**：支持在构建阶段按需注入 `Hybrid Mount`、`SuSFS` 与 `BBG`。Hybrid Mount 适用于所有内核，在自动构建中默认关闭并可通过 workflow 选项启用；`SuSFS` 仅在 `ReSukiSU` 构建中启用，BBG 仅对声明支持的项目可用。
...
* **Apply Hybrid Mount / SuSFS / BBG**: workflow 中 Hybrid Mount 与 BBG 默认关闭，SuSFS 默认开启；可按需勾选 Hybrid Mount。其中 Hybrid Mount 适用于所有项目，`SuSFS` 仅在 `ReSukiSU` 构建时真正生效。

...
* **Apply SuSFS / Apply BBG**: `SuSFS` 默认关闭，`BBG` 默认开启；其中 `SuSFS` 仅在 `ReSukiSU` 构建时真正生效。
...
上游监听每 3 天自动运行一次；`Build All Projects` 每 7 天自动构建一次包含 `SuSFS` 的全量版本，其余自动和手动构建默认不启用 `SuSFS`。
...
# ReSukiSU 默认启用 BBG；如需 SuSFS 可显式传参
./kokuban build s25_sm8750 resukisu resukisu --with-susfs
...
本地 CLI 默认会给构建传入 `apply_susfs=false` 与 `apply_bbg=true`。也可以写入本地默认配置：

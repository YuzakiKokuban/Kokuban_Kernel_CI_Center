...
* **动态特性注入**：支持在构建阶段按需注入 `SuSFS`、`BBG`、`Re-Kernel` 与 `zram` LZ4 NEON 支持，其中 `SuSFS` 仅在 `ReSukiSU` 构建中启用。
...
* **Apply SuSFS / Apply BBG / Apply Re-Kernel / Apply zram**: 默认开启；其中 `SuSFS` 仅在 `ReSukiSU` 构建时真正生效。
...
# ReSukiSU 默认启用 SuSFS、BBG、Re-Kernel 与 zram，如需临时关闭可显式传参
./kokuban build s25_sm8750 resukisu resukisu --no-susfs --no-bbg --no-rekernel --no-zram
...
```bash
./kokuban list                 # 列出可用项目
./kokuban features             # 查看所有项目的 SuSFS / BBG / Re-Kernel / zram 支持状态
./kokuban features s25_sm8750  # 查看单个项目的 SuSFS / BBG / Re-Kernel / zram 配置
...
本地 CLI 默认会给构建传入 `apply_susfs=true`、`apply_bbg=true`、`apply_rekernel=true` 与 `apply_zram=true`。也可以写入本地默认配置：
...
./kokuban config set apply_zram true
...
每次构建还会写入 `build-info.json`，记录项目、源码提交、SuSFS / BBG / Re-Kernel / zram 状态、主机信息、工具链缓存路径与 SHA-256。构建失败时，CLI 会从日志中提取关键错误行和最后一段日志，直接打印失败摘要。

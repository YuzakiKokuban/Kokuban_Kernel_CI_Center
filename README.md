...
`Watch Upstream KernelSU` 只对配置了 `watch_upstream_variants` 的项目生效；当前启用了 `s25_sm8750`、`s23_sm8550`、`tabs9_sm8550`、`s24_sm8650`、`tabs10_mt6989`、`mi17_sm8850`，以 [configs/projects.json](configs/projects.json) 为准。
...
# ReSukiSU 默认启用 SuSFS 与 BBG，如需临时关闭可显式传参
./kokuban build s25_sm8750 resukisu resukisu --no-susfs --no-bbg
...
本地 CLI 默认会给构建传入 `apply_susfs=true` 与 `apply_bbg=true`。也可以写入本地默认配置：
